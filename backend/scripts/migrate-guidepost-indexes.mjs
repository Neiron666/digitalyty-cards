...
import GuidePost from "../src/models/GuidePost.model.js";
...
async function checkDuplicateSlugs(verbose) {
    ...
    const dupes = await GuidePost.aggregate(pipeline);
...
async function ensureIndexes({ dryRun, verbose }) {
    let idx = [];
    try {
        idx = await GuidePost.collection.indexes();
    } catch (err) {
    ...
    if (byName.has(wantSlug)) {
        ...
        if (hasDuplicates) {
            ...
            if (!dryRun) {
                await GuidePost.collection.createIndex(
                    { slug: 1 },
    ...
    if (!dryRun) {
        const after = await GuidePost.collection.indexes();
