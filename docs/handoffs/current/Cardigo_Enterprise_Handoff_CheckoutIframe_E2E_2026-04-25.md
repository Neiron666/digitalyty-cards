...
## 9) Open P2 Tails (closed and deferred)

| #    | Tail                                                                                    | Location                                              | Status / Risk                                                                                                                                                                                                                                                                   |
| ---- | --------------------------------------------------------------------------------------- | ----------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| P2-1 | ~~`handleReceiptSave` sends a no-op PATCH if no profile fields changed~~                | `CheckoutPage.jsx`                                    | **CLOSED (MC-H1, 2026-04-26).** Empty-payload guard in `handleReceiptSave` after validation, before `setReceiptBusy(true)`. No PATCH sent on no-op. Message: "לא בוצעו שינויים."                                                                                                |
| P2-2 | ~~`receiptOk` not cleared in no-diff path of `handleContinueToSummary`~~                | `CheckoutPage.jsx`                                    | **CLOSED (MC-H1, 2026-04-26).** `setReceiptOk("")` on no-diff advance. `setPaymentError("")` on fail back-to-summary.                                                                                                                                                           |
| P2-3 | `handleExternalFallback` may create orphaned external-mode PI when `paymentUrl` is null | `CheckoutPage.jsx`                                    | Orphaned PI expires via 2h TTL. No security or billing risk. _(deferred)_                                                                                                                                                                                                       |
| P2-4 | ~~HTML tags not stripped from `name`/`nameInvoice`/`address` before YeshInvoice~~       | ~~`tranzila.provider.js`~~ → `yeshinvoice.service.js` | **CLOSED (MC-H2, 2026-04-26).** `stripHtmlForYeshInvoiceText()` in `yeshinvoice.service.js`. Applied to `Name`, `NameInvoice`, `FullName`, `Address`, `City`. Does not strip `EmailAddress`, `NumberID`, `ZipCode`, `CountryCode`. No DB mutation.                              |
| P2-5 | Pricing CTA UX for active subscribers                                                   | `CheckoutPage.jsx` / Pricing                          | Active subscriber sees notice + redirect; no upgrade option visible. _(deferred)_                                                                                                                                                                                               |
| P2-6 | SettingsPanel receipt-profile no-op save triggered confusing backend 400 error          | `SettingsPanel.jsx`                                   | **CLOSED (MC-S1, 2026-04-26).** Empty-payload guard in `handleReceiptProfileSave` after validation, before `setReceiptProfileBusy(true)`. Message: "לא בוצעו שינויים." No receipt cabinet or payment flow change.                                                               |
| P2-7 | Payment pages (`CheckoutPage`, `IframeReturnPage`) had no Cardigo brand identity        | `CheckoutPage.jsx`, `IframeReturnPage.jsx`            | **CLOSED (MC-H4, 2026-04-26).** `CheckoutBrandMark` in all 10 checkout states + `IframeReturnPage`. Assets: `/images/brand-logo/cardigo-logo.webp` + `.png` fallback. Non-clickable `<div>`. No `Link`/`onClick`/`Header`/`Layout`. No payment, postMessage, or sandbox change. |
