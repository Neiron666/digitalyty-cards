...
11. ORG_MEMBER_INDEX_CI_BOOTSTRAP_AND_PROD_PARITY — **CLOSED / VERIFIED (2026-05-04)**.
    CI bootstrap gap fixed: `cardigo_ci.organizationmembers` was missing `orgId_1_userId_1` unique compound index, causing `sanity:org-membership` to fail at preflight with `NamespaceNotFound`. `migrate-orgmember-indexes.mjs --apply --verbose` run against `cardigo_ci` — index created, both `sanity:org-membership` and `sanity:org-access` EXIT:0. Production parity separately verified read-only (dry-run, no `--apply`) against `cardigo_prod` — `orgId_1_userId_1 already exists and is unique — no-op`. No production apply was performed or required. CI and production are now at parity. Bootstrap runbook (`docs/runbooks/ci-cluster-bootstrap.md`) updated: §5 Step 5 added, §9 forbidden-operations row corrected, §11.5 closure entry added. `backend/README.md` Index Governance section updated. `docs/runbooks/backend-verification-and-deploy.md` sanity:org-membership prerequisite note added.

12. SANITY_INVITE_HELPERS_DEDUP — **CLOSED / VERIFIED (2026-05-04)**.
    Deduplicated `SANITY_INVITE_PASSWORD` and `extractTokenFromInviteLink` into `backend/scripts/sanity-shared-fixtures.mjs` as the single source of truth. `sanity-org-access.mjs` and `sanity-org-membership.mjs` now import both helpers; local copies were removed. The robust token parser behavior (7-guard implementation) was preserved. Narrow scope by design: other per-script helpers remain local. Static verification PASS, fake-production guard PASS, and both `sanity:org-membership` + `sanity:org-access` passed EXIT:0 against safe `cardigo_ci` with `NODE_ENV=development`. Production DB was not touched. No runtime/frontend/package/CI changes.
