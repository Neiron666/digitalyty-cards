...
## 9) Новый закрытый milestone: permanent blocked re-registration after self-delete
...
Бизнес-требование было такое:

1. пользователь перед удалением должен понимать, что этот email больше нельзя будет использовать для нового аккаунта;
2. система должна реально запрещать повторное создание аккаунта с этим email;
3. решение должно быть enterprise-grade, с минимальным раздуванием БД и минимальным blast radius.

### 9.2 Какое архитектурное решение было выбрано

Выбран взрослый bounded вариант:

**hard delete + dedicated deleted-email tombstone/block collection**.

Почему не soft-delete в `users`:

- это раздувает `users` полными мёртвыми документами;
...
Почему tombstone collection правильнее:

- минимальный footprint;
- не загрязняет `users`;
- хорошо отделяет active identity truth от deleted identity truth;
- легко и явно проверяется в create-user paths;
- соответствует enterprise identity-retention discipline.
...
- снижает риск dictionary-style recovery;
- даёт privacy-preserving deterministic lookup;
...
1. пройти guards;
2. загрузить email;
3. вычислить normalized email;
4. записать tombstone;
5. только потом запускать destructive cascade.

Это **tombstone-first** ordering.

Почему:

- tombstone-first = fail-safe;
- delete-first = fail-open.

Если tombstone write не проходит (не duplicate-key case):
...
- signup-consume → тот же fail/neutral 400 path
...
После удаления пользователь видит финальное сообщение:

- аккаунт удалён;
- этот email больше не будет доступен для нового аккаунта.

При попытке повторной регистрации пользователь видит понятный Hebrew UX message вместо сырого `Unable to register`.

### 9.4 Что было подтверждено runtime’ом

Подтверждено руками:

- tombstone document реально пишется в `deletedemailblocks`;
- `emailKey`, `formerUserId`, `createdAt` присутствуют;
- повторная обычная регистрация тем же email блокируется;
- signup-link не создаёт usable new-account path;
- старые одноразовые ссылки после delete больше не приводят к новому аккаунту;
- finality UX message показывается;
- post-delete redirect delay поднят до 3000ms как UX polish.

### 9.5 Что intentionally НЕ включено

Осознанно оставлено вне scope:

- `admin-delete` policy;
- deeper token/auth cleanup contours;
- broader verification-link lifecycle redesign;
- any broader identity-lifecycle cleanup outside this bounded milestone.

### 9.6 Что теперь считать закрытым truth
...
1. **email verification / old verification link behavior after deleted/blocked lifecycle**  
   Это уже отдельный auth/verification contour, не часть только что закрытого milestone.

2. **payment notify / webhook audit**  
   Но только если Tranzila уже близка к реальной интеграционной truth.

3. **gate / public launch strategy**  
   Если проект подходит к моменту real public exposure.
...
- reopen-ить закрытый self-delete email-block contour без новой реальной причины.

---

## 11) Как должен работать следующий чат GPT
