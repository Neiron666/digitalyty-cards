...
| `emailMarketingConsent` field on User model                                  | ✅ Implemented          |
| Opt-in checkbox — registration, signup-consume, invite-accept                | ✅ Implemented          |
| EditorSidebar trial nudge (yes/no consent prompt)                            | ✅ Implemented          |
| SettingsPanel “עדפות תקשורת” toggle + `PATCH /api/account/email-preferences` | ✅ Implemented          |
| `MarketingOptOut` model + HMAC suppression util                              | ✅ Implemented          |
| `EmailUnsubscribeToken` model + shared TTL util                              | ✅ Implemented          |
| Unsubscribe token generation in job (Step 5)                                 | ✅ Implemented          |
| Multipart email with unsubscribe footer (TextPart + HTMLPart)                | ✅ Implemented          |
| `POST /api/unsubscribe/consume` — atomic token consume + suppression write   | ✅ Implemented          |
| Frontend `/unsubscribe` page (`Unsubscribe.jsx`)                             | ✅ Implemented          |
| Governed compound index (`trialReminderSentAt_1_trialEndsAt_1` on users)     | ✅ Applied (2026-04-12) |
| Governed indexes on `emailunsubscribetokens` (4 indexes)                     | ✅ Applied (2026-04-12) |
| Governed index on `marketingoptouts` (`emailKey_1` unique)                   | ✅ Applied (2026-04-12) |
...
| `TRIAL_REMINDER_ENABLED` production gate (default `false`)                   | ✅ Implemented          |
| Smoke test + frontend gates                                                  | ✅ Passed (2026-04-12)  |
...
> **Контур технически завершён и верифицирован. Роллаут намеренно заблокирован до явного операторского решения.**

`TRIAL_REMINDER_ENABLED` сейчас `false` в продакшне. Джоб стартуется, но немедленно выходит без планирования. Изменение на `true` не является изменением кода — это исключительно решение оператора. См. предусловия и инструкции в `docs/runbooks/trial-lifecycle-ssot.md` §13.11.

Следующее действие — операторское решение о включении, не архитектурная работа.
