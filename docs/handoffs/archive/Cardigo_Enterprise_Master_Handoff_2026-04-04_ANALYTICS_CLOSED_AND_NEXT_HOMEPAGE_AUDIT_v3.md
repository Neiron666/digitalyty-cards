...
ручная DB/index governance вместо хаотического runtime-роста. 2) Текущий стек и инфраструктура
2.1 Frontend
React + Vite
RTL-first
CSS Modules only
...
добавлены idx_leads_mailbox и idx_leads_deletedAt_ttl. 4) Роль ChatGPT и роль Copilot Agent
4.1 Роль ChatGPT
...
не трогать соседние контуры без доказанной необходимости. 5) Каноническая рабочая доктрина
5.1 Workflow
...
definition of done;
deliverable format;
STOP после нужной фазы. 6) Жёсткие архитектурные инварианты проекта
...
proxy/gate assumptions 7) Что уже было важно закрыто раньше и не должно casually reopen-иться
...
ставит auth cookie
response truth:
{ ok: true, orgId, orgSlug } 9) Self-delete finality / permanent blocked re-registration
9.1 Проблема
...
затем снова зарегистрировать новый аккаунт на тот же email.
...
в карточках перестали корректно считаться просмотры/клики;
write-path был заблокирован security/transport contour;
затем был найден и data-parity contour по action normalization;
затем был найден malformed-JSON parity gap;
после code fix был выполнен runtime verification и docs alignment.
14.2 Что именно было доказано на audit
Write-path root cause

Card analytics write-path ломался из-за комбинации двух вещей:

cookie-backed auth + csrfGuard
logged-in browser session посылала auth cookie;
analytics POST шёл через sendBeacon() / fetch() без X-Requested-With;
csrfGuard блокировал запрос.
proxy gate bypass drift
bypass уже существовал для POST /api/site-analytics/track;
но отсутствовал для POST /api/analytics/track.
Read-path truth

Owner/UI/read-model не были primary defect.
UI честно показывал нули, потому что ingest path не писал события.

14.3 Что было исправлено в коде

Закрытый contour включает эти файлы:

csrf.middleware.js
добавлено узкое CSRF exemption для:
POST /api/analytics/track
POST /api/site-analytics/track
proxy.js
gate bypass расширен и на:
POST /api/analytics/track
analytics.controller.js
backend normalizeAction allowlist дополнен:
booking
twitter
analytics.client.js
frontend normalizeAction allowlist дополнен:
booking
twitter
app.js
malformed-JSON 204 parity выровнена и для:
POST /api/analytics/track
POST /api/site-analytics/track
14.4 Что было доказано в verification
Production/runtime proof

Доказано:

POST /api/analytics/track на production возвращает 204;
POST /api/site-analytics/track на production возвращает 204;
non-exempt route без gate-cookie продолжает быть закрытым;
браузерный POST /api/analytics/track идёт с 204, а не 403;
просмотры, клики и attribution снова растут.
Product truth

Также было подтверждено важное product decision:

в текущем скоупе не нужна granular action-level аналитика в UI;
важна общая кликовая активность и источники/кампании/платформы;
passive UI toggles не должны засорять analytics noise.
14.5 Что было отдельно проаудировано как coverage truth

Был отдельный audit по tracking coverage на карточке.
...
там только passive toggles / accordion-open,
нет strong business-intent signal,
их tracking дал бы noise, а не ценность.
Уже трекается и этого достаточно:
meaningful contact actions;
booking;
lead;
внешние CTA clicks;
platform/source attribution.
14.6 Что обновлено в docs

После code + verification был сделан docs pass и truth aligned в живых operational docs:

backend.instructions.md
SECURITY_TEMP_API_GATE.md
copilot-instructions.md

Теперь в docs отражены:

analytics write endpoints как best-effort / always-204;
bounded CSRF exemption;
bounded proxy gate bypass;
malformed-JSON 204 swallow parity;
normalizeAction allowlist truth (booking, twitter и др.).
14.7 Что считать закрытым truth

Analytics contour теперь считается полностью закрытым:

code
verification
production-oriented smoke
docs truth alignment
14.8 Что осталось только как non-blocking note

Не блокирует closure:

historical data gap за период поломки;
share/contact naming overlap;
inline style в SocialButtons.

Это не открытые хвосты, а просто known notes.

15. Что не нужно делать сейчас в analytics

Не нужно сейчас:

добавлять tracking на every accordion/open-state;
строить overly granular action-level analytics UI;
делать broad taxonomy redesign;
reopen-ить write-path contour;
мешать analytics с unrelated admin/public redesign.

Правильный продуктовый фокус сейчас:

видеть откуда пришёл трафик;
видеть, есть ли visits / clicks / results;
не создавать noise-heavy analytics. 16) Как принимать или не принимать работу Copilot
...
требование RAW stdout + EXIT для verification. 18) Практические правила следующего окна GPT
...
Не нужно reopen-ить analytics contour.
Не нужно тянуть новый marketing/UI redesign без аудита.
Не нужно уходить в noisy action-level analytics.

19.2 Что брать следующим шагом
...
Helper-copy / title-description semantics
...
Schema / JSON-LD / structured assist
...
19.3 Почему именно этот contour
...
- bounded exception exists for public analytics write endpoints:
    - POST /api/analytics/track
    - POST /api/site-analytics/track
- CORS contract:
    - explicit allowlist from CORS_ORIGINS
...
- public analytics write-path contour is closed
- POST /api/analytics/track is public best-effort always-204 style
- POST /api/site-analytics/track is public best-effort always-204 style
- bounded proxy gate bypass exists for both analytics write endpoints
- malformed JSON on both analytics write endpoints is swallowed to 204
- action normalization now preserves booking and twitter on both frontend and backend
- analytics should remain focused on meaningful actions and source/platform/campaign truth, not passive accordion noise
- do not reopen analytics write-path contour casually
...
с полностью закрытым analytics contour:
write-path fixed,
