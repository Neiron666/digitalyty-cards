...
rollback возвращает exact pre-claim/pre-trial shape.

---

### 10.3 Rollback silent failure / observability gap
**Симптом:**  
Если user save падал, а compensating rollback тоже падал, это раньше проглатывалось молча.

**Fix:**  
Добавлен явный critical log о возможном inconsistent card state.

**Что intentionally deferred:**  
Полная атомарность через транзакции — отдельный будущий contour, не хвост этого scope.
...
### 13.1 Full atomicity / transactions for claim flow
Сейчас:
- rollback shape и observability исправлены,
- но claim flow не переведён на Mongo transactions.

Почему это не делалось сейчас:
- это уже отдельный архитектурный contour;
- потребует transaction pattern, session discipline, инфраструктурной проверки;
- это не хвост текущего bounded fix.
...
- possible transactional hardening for claim flow in future;
