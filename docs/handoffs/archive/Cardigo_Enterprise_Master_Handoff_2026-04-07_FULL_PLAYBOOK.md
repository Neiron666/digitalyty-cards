...
- PWA install contour (manifest, shared runtime store, site footer CTA, card footer CTA, lifecycle fix).
...
### 8.9 PWA install contour — CLOSED

Закрыто:

- `manifest.webmanifest` — создан, прилинкован из `index.html`, `scope: "/"`, `start_url: "/"`;
- shared install prompt runtime store (`installPromptStore.js`) — инициализируется на уровне модуля в `main.jsx`, до любых lazy-route и async data fetch;
- `useInstallPrompt` hook переведён на `useSyncExternalStore` — подписчик store-а, не владелец событий;
- site footer CTA (`InstallCta.jsx`) — видна на всех Layout-wrapped routes;
- card footer CTA (`CardFooter.jsx` → `InstallRow`) — видна на всех публичных карточках, скрыта в preview;
- lifecycle fix: bidirectional display-mode MQL, re-sync на pageshow/visibilitychange/focus, без optimistic accepted→installed;
- timing gap на card routes устранён: `beforeinstallprompt` захватывается до рендера React tree.

Ключевая product truth:

- установка Cardigo с любого route (homepage или card footer) устанавливает **один** Cardigo app (`start_url: "/"`);
- per-card отдельный PWA не реализован и не планируется в этом contour-е.

Канонический durable doc: `docs/pwa-install-architecture.md`
...
Cardigo сейчас находится в сильной зрелой точке: это enterprise-minded Israel-first SaaS для цифровых визиток с ручной DB/index governance, строгой архитектурной дисциплиной, закрытым trial/free/premium lifecycle contour, корректным claim/recovery path, работающим forced expiry/free fallback, retention purge с premium-data cleanup, реализованной claim-flow transaction atomicity через native Mongo transaction, чистой truth по owner inbox leads, исправленным stale-state после archive/trash/unarchive/unstar, persistent important star indicator в lead rows, дополнительной polish-правкой mobile drawer icons для `/inbox` и `/edit`, закрытым privacy/consent contour с fixed two-view banner, `Manage preferences`, footer reopen path и aligned privacy policy, и закрытым PWA install contour с shared runtime store, site/card footer CTAs и корректным lifecycle на всех routes. Следующий GPT должен не "чинить всё подряд", а идти bounded contours, держать architecture truth, соблюдать enterprise workflow и выбирать safest mature path over fastest hack.
