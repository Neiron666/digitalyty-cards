...
- `backend/src/services/yeshinvoice.service.js` — `createReceiptYeshInvoice` + `shareReceiptYeshInvoice`. Auth: `Authorization: JSON.stringify({ secret, userkey })` (literal JSON string). Both operations use uppercase response fields `Success`, `ErrorMessage`, `ReturnValue`, but the `ReturnValue` shape differs by operation — **do not collapse these into one shape**:
    ...
    - **Anti-drift:** Do not add extra fields to the `shareDocument` request body. Do not treat `Success: true` as equivalent to `ReturnValue: true` for share operations. (Phase 2A fix, 2026-05-02.)
...
- On share failure: `Receipt.shareStatus` set to `"failed"`; `shareFailReason` recorded. Two structured warn-level log events are emitted by `tranzila.provider.js` and serve as operator log anchors on Render:
    - `receipt_share_failed` — fired when `shareResult.ok === false` for any reason, including `Success: true` with `ReturnValue: false/missing` (reason: `share_document_not_sent`) and missing `providerDocId` (reason: `missing_provider_doc_id`). Logged in both `first_payment` and `sto_recurring` flows. Contains only non-sensitive boolean presence fields — never raw provider IDs, document URLs, raw customer emails, auth values, or raw provider payloads.
    - `receipt_share_exception` — fired as the first statement in the share IIFE `catch` block when an unexpected throw occurs. Same sensitive-field restrictions apply. Logged in both flows.
    - Neither event is fired on `Receipt.create()` idempotency (E11000) — those are handled separately.
