...
- **Resume auto-renewal** is available self-service in the product UI (SettingsPanel → billing section) via `POST /api/account/resume-auto-renewal` (production-proven 2026-05-19). Requires `tranzilaSto.status === "cancelled"` and a valid stored Tranzila token. Token-absent/expired state is support-mediated. See `billing-flow-ssot.md §19`.
...
- **Self-service resume auto-renewal is available in the product UI** (SettingsPanel billing section, production-proven 2026-05-19 — two monthly-plan production users resumed; new STO schedules at ₪29.00; next billing 05/06/2026). User path: **חדש חידוש אוטומטי** button (shown only when `autoRenewal.status === "cancelled"` AND subscription still active AND `expiresAt` not yet passed AND provider is Tranzila AND plan is monthly/yearly) → `POST /api/account/resume-auto-renewal` → `createTranzilaStoForUser` with `allowRecreateAfterCancel:true`. Requires a valid stored Tranzila token. Token-absent/expired state is support-mediated. See `billing-flow-ssot.md §19`. Resume creates a new STO schedule only — no `PaymentTransaction` or `Receipt` is created. **Operator-side counterpart:** `backend/scripts/sto-resume-after-cancel.mjs` — dry-run default, `--execute` requires an explicit confirmation flag, production terminal requires `--allow-prod`. See script JSDoc for exact CLI. Note: `cancellation_*` fields are retained as historical audit metadata after resume; use `tranzilaSto.status` (not `cancelledAt`) to determine current STO state. See `billing-flow-ssot.md §19 "STO Cancellation Audit Retention After Resume"`.
...
1. ~~**Self-service portal**~~ — **RESOLVED (5.9a.1/5.9a.2 + 2026-05-02; production-proven 2026-05-19):** User self-service cancel-renewal shipped. Resume auto-renewal production-proven (2026-05-19). Delete saved payment method smoke-verified. Payment-method replacement (updating to a new card) and refunds remain support-mediated.
