...
| `MARKETING_SEND_TO_LIST_ENABLED` | no       | absent = disabled                                                        | Set `"true"` to allow admins to queue marketing campaigns via `POST /api/admin/marketing/:id/send-to-list`. Absent or any other value returns `409`. Does not itself send email — the real-send worker is controlled separately by `MARKETING_REAL_SEND_WORKER_ENABLED`. Must be absent/false in production unless admins should be able to queue new campaigns. |
