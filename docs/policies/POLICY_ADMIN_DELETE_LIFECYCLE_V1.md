...
| SlugRedirect (slug tombstones) | **CREATE** slug quarantine tombstone per deleted card | Via `createCardSlugTombstone` — only for user-owned cards with a claimed personal slug; anonymous/user-null cards skipped. Reason: `card_deleted` (card controller) or `account_deleted` (account-delete cascade). Fields: `status: "redirect_quarantine"`, `targetCardId: null`, `expiresAt: now+30d`. Records are **retained** as audit trail — never deleted on user deletion. |
...
| SlugRedirect (slug tombstone) | `slugredirects`        | **CREATED** on card delete — not deleted on user deletion                                                    | **RETAIN** (audit trail)                                           | `createCardSlugTombstone` writes tombstone per deleted user-owned card with a claimed personal slug; anonymous/user-null cards skipped | Slug audit trail; blocks reclaim during quarantine; released by sweep after ~30d; public result is plain 404; no PII | No — tombstone contains no user-identifiable data beyond `sourceCardId`                 |
